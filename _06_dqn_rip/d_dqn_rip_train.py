...
def main():
    env_config = {
        ...
        "terminate_motor_angle_deg": 100.0,      # 종료 조건 모터 각도
        "terminate_pendulum_angle_vel": 40.0     # 종료 조건 펜듈럼 각속도
    }
    config = {
        "env_name": "quanser",                           # 환경의 이름
        "max_num_episodes": 4000,                        # 최대 학습 에피소드 수
        ...
        "epsilon_start": 0.95,                           # ε-탐욕 시작값
        "epsilon_end": 0.1,                              # ε-탐욕 종료값
        "epsilon_final_scehduled_percent": 0.025,        # ε 선형감소 종료 지점(전체 에피소드 비율)
        "training_start_steps": 256,                     # 학습 시작 최소 스텝(버퍼 워밍업)
        "forward_start_episode": 25,                     # 정책 실행(ε-탐욕) 시작 에피소드
        ...
        "validation_start_episode": 50,                  # 검증 시작 에피소드
